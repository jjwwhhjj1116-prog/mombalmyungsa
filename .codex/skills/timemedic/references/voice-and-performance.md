...
- 몸의 발명사 기본 화자는 Typecast의 `필재`다. 특정 실제 성우의 음색을 복제하지 않고 Typecast가 제공하는 정식 캐릭터를 사용한다.
- 1차 오디션은 속도 `1.0`과 기본 감정으로 만든다. 참고 영상에서 별도 속도·피치 수치가 확인되지 않았으므로 임의 수치를 운영 규칙으로 꾸미지 않는다.
- 대본 전체를 합성하기 전에 훅, 질문, 딜레마, 반전, 결말 문장으로 짧은 오디션을 만든다.
...
- 수익화 채널에는 상업 이용이 허용되는 Typecast 요금제와 다운로드 조건을 확인한다. 무료 플랜 음성을 상업용으로 사용하지 않는다.
- ElevenLabs audition take 2는 Typecast 결과와 비교하는 톤 레퍼런스다. 사용자가 다시 명시적으로 선택하지 않으면 전체 나레이션의 기본값으로 복귀시키지 않는다.
...
- 기본은 Designed Voice + `Natural`이다.
- 태그는 전환점에만 쓴다: `[curious]`, `[tense]`, `[sighs]`, `[excited]`.
- SSML break 대신 줄바꿈, 쉼표, 말줄임표로 정적을 만든다.
...
- 질문 전환 직전 180–320밀리초의 체감 정적을 만들고 `발상을 뒤집습니다`는 오히려 반 박자 느리게 읽는다.
- 수치·치수·연도는 속도를 낮춰 자음을 분명히 하고, `이렇게 탄생했습니다`는 웅장함보다 낮고 따뜻한 확신으로 끝낸다.
...
- 모든 분할은 동일한 voice ID, model, speed, stability/style 설정과 문맥 요약을 사용한다. 경계 전후를 실제로 듣고 음색·속도·호흡 점프가 하나라도 있으면 전체 또는 해당 큰 구간을 다시 합성한다.
