...
- 몸의 발명사에 관한 `앞으로`, `항상`, `무조건`, `규칙화`, `스킬화` 지시는 같은 작업 안에서 이 저장소의 문서·스킬·검증 규칙에 반영한다.
- 반영 후 `scripts/pipeline.py validate --all`을 통과시키고, 커밋과 원격 `main` 푸시까지 완료해야 지침 변경이 끝난 것으로 본다.
- 푸시가 실패하면 규칙이 적용됐다고 보고하지 않고 `rule_change_unpushed` blocker를 남긴다.
- 에피소드별 결과물과 lock도 단계가 통과할 때마다 커밋·푸시한다. MP3·MP4 같은 대용량 바이너리는 저장소에 올리지 않고 SHA-256 영수증과 생성 기록만 올린다.
...
- Flow 프롬프트는 실제 키 입력과 서버 반영 카드의 이중 일치 검사를 통과해야 한다. 붙여넣기·`fill`·DOM 주입으로 화면에만 보인 프롬프트는 유효한 제출로 인정하지 않는다.

## 현재 체크포인트

첫 이관 에피소드는 `까스활명수 V6`다. 사용자 승인 대본·문장별 스토리보드·`까스 / 없는 / 활명수` 썸네일 계약·Omni 생성 계획과 U01·U10 파일럿이 잠겼고, 현재 단계는 `09_batch_generation`이다. U02–U06은 시각 QA를 통과했으며 U07은 Flow의 비정상 활동 차단으로 안전 재시도 대기 중이다. 구버전 TTS와 구버전 영상은 폐기 대상으로 명시되어 있다.

모든 에피소드는 `대본 승인 → 썸네일 픽셀·모바일 QA·무문자 시작 이미지 잠금 → 영상 생성` 순서를 지킨다. 썸네일은 업로드 직전에 덧붙이는 장식이 아니라 영상 제작을 여는 선행 게이트다.
